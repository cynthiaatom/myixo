...
export default function DecisionLab(){
  ...
  return <section className="intelligenceView">
    <div className="viewHero"><div className="eyebrow">DECIDE · DECISION LAB</div><h1>SEE THE DECISION <em>CLEARLY.</em></h1><p>iXo helps expose context, assumptions, unknowns and tradeoffs—not choose for you.</p></div>
    <div className="decisionLayout"><div className="decisionForm">
      ...
      <label>DESIRED OUTCOME · OPTIONAL<textarea value={input.desiredOutcome} onChange={e=>setInput({...input,desiredOutcome:e.target.value})} placeholder="What would a good outcome accomplish?"/></label>
      <label>OPTIONS · OPTIONAL · ONE PER LINE<textarea value={optionsText} onChange={e=>setOptionsText(e.target.value)} placeholder={'Option A\nOption B'}/></label>
      <label>ASSUMPTIONS TO TEST · OPTIONAL · ONE PER LINE<textarea value={assumptionsText} onChange={e=>setAssumptionsText(e.target.value)} placeholder={'This will require...\nI am assuming...'}/></label>
      <button className="primary" disabled={working||!input.decision.trim()} onClick={analyze}>{working?'iXo is thinking...':'Think this through →'}</button>
    ...
    <div className="decisionResult">{result?<><div className="eyebrow">DECISION ANALYSIS · GROUNDED IN iXo CONTEXT</div><h2>{String(result.summary||'Decision context')}</h2>
      ...
      <details className="contextUsed"><summary>Personal context used · {evidence.length}</summary>{evidence.map(f=><p key={f.id}><b>{f.category}</b> · {f.value}</p>)}</details>
