import type {Explanation,MemoryFact} from '../intelligence/types';

export default function ExplainPanel({explanation,onClose,onCorrect}:{explanation:Explanation;onClose:()=>void;onCorrect?:(fact:MemoryFact)=>void}){
  return <div className="explainBackdrop" onMouseDown={onClose}><aside className="explainPanel" onMouseDown={e=>e.stopPropagation()}>
    ...
    {onCorrect&&<button className="primary" onClick={onClose}>Correct my iXo in Memory →</button>}
  </aside></div>
}
