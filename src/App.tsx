...
export default function App(){
  ...
  const currentPlaceholder=user?'Tell iXo in your own wordsÃ¢â‚¬Â¦':questions[demoQi]?.placeholder;
  ...
  return <main><div className="veil"/><header><div className="logo"><span className="my">MY</span><span className="ixo">iXo</span></div><div className="demo">{user?'iXo CONNECTED':'EXECUTIVE DEMO'}</div></header><div className="wrap">
    ...
    {memoryLoading&&<div className="contextLoading">Loading your iXo contextÃ¢â‚¬Â¦</div>}
    {stage==='hero'&&initState==='LOADING'&&<section className="hero loadingHero"><div><div className="eyebrow">PERSONAL INTELLIGENCE, MADE VISIBLE</div><h1>BUILD YOUR <em>iXo</em></h1><p className="lead">Syncing your real iXo context before showing your Personal Map.</p><div className="syncPill"><span/>Checking session Ã‚Â· loading native Personal Map</div></div><div className="mapLoading"><div className="mapLoadingCore">MY<br/><b>iXo</b></div><p>SYNCING PERSONAL MAP</p></div></section>}
    {stage==='hero'&&(initState==='PARTIAL'||initState==='ERROR')&&user&&<section className="hero"><div><div className="eyebrow">CONNECTED Ã‚Â· PERSONAL MAP UNAVAILABLE</div><h1>BUILD YOUR <em>iXo</em></h1><p className="lead">Connected to iXo, but the Personal Map couldn't be loaded.</p><div className="actions"><button className="primary" onClick={retryPersonalMap}>Retry Personal Map</button><button onClick={()=>setStage('connect')}>Connection</button></div><p className="note">Your native Personal Map is unavailable right now. Durable Memory is separate and is not being used as a replacement.</p></div></section>}
    {stage==='hero'&&(initState==='READY'||initState==='AUTH_REQUIRED')&&(!user||mapReady)&&<section className="hero"><div><div className="eyebrow">PERSONAL INTELLIGENCE, MADE VISIBLE</div><h1>BUILD YOUR <em>iXo</em></h1><p className="lead">Turn what iXo knows about you into an intelligent personal operating system.</p><div className="stat"><strong>{progress.label}</strong><div><b>{progress.description}</b><p>{model.mode==='areas'?('iXo currently has saved context in '+progress.current+' of 10 Personal Map areas.'):('Executive demo: '+progress.current+' of 30 illustrative context dimensions.')}</p></div></div><div className="actions"><button className="primary" disabled={briefingLoading} onClick={begin}>{briefingLoading?'Preparing iXoÃ¢â‚¬Â¦':user?'Continue My Personal Map Ã¢â€ â€™':'Connect My iXo Ã¢â€ â€™'}</button><button onClick={()=>setStage('demo')}>Executive Demo</button>{user&&<button disabled={memoryLoading} onClick={openMemory}>{memoryLoading?'Loading MemoryÃ¢â‚¬Â¦':'Memory Inspector'}</button>}{user&&<button onClick={()=>setStage('connect')}>Connection</button>}</div><p className="note">The Personal Map shows where iXo has saved context. It is never a rating of your life.</p></div><Map model={model}/></section>}

    {stage==='demo'&&<section className="flow"><div className="panel">{phase==='thinking'?<div className="center"><div className="eyebrow">LIVE iXo STATUS</div><div className="statusSteps">{liveStatuses.map((s,i)=><div key={s} className={i<processStep?'complete':i===processStep?'current':''}><span>{i<processStep?'Ã¢Å“â€œ':i+1}</span><b>{s}</b></div>)}</div><div className="scan"/></div>:phase==='reveal'?<div className="center"><div className="eyebrow">PERSONAL MAP UPDATED</div><h2>{changed.length?('iXo added context to '+changed.length+' '+(changed.length===1?'area':'areas')):'Your answer added useful context to iXo'}</h2><div className="jump">{previousLabel} Ã¢â€ â€™ {mapProgress(model).label}</div>{changed.length>0?<div className="learned">{changed.map(c=><div key={c}><b>{labels[c]}</b> Ã‚Â· new saved context</div>)}</div>:<p className="helper">The answer was saved, but it did not create a newly covered Personal Map area. iXo can still use the context.</p>}<button className="primary" onClick={()=>setPhase('ask')}>Continue Ã¢â€ â€™</button></div>:<><div className="eyebrow">{questionLabel} Ã‚Â· {progress.label} {model.mode==='areas'?'AREAS':'DIMENSIONS'} UNDERSTOOD</div>{currentPrompt?<><h2>{currentPrompt}</h2><p className="helper">{currentHelper}</p><div className="chips">{chips.map(c=><button key={c.id} onClick={()=>user?toggleOption(c.id):setAnswer(c.label)} className={(user?selected.includes(c.id):answer===c.label)?'selected':''}>{c.label}</button>)}</div>{(!user||question?.allow_custom!==false)&&<textarea value={answer} onChange={e=>setAnswer(e.target.value)} placeholder={currentPlaceholder}/>} {connectError&&<div className="connectError">{connectError}</div>}<div className="actions"><button className="primary" onClick={()=>user?sendNative(false):sendDemo()}>Send to iXo Ã¢â€ â€™</button>{user&&question?.allow_skip&&<button onClick={()=>sendNative(true)}>Skip</button>}{!user&&<button onClick={()=>{setDemoQi(Math.min(demoQi+1,questions.length-1));setAnswer('')}}>Skip</button>}</div></>:<div className="center"><div className="thinking">{session?.generation?.state==='generating'?'iXo is preparing your next questionÃ¢â‚¬Â¦':'No question is available yet.'}</div></div>}</>}</div><Map model={model} highlight={changed}/></section>}
