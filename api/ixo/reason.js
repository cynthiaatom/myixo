...
const safeMessage=value=>String(value||'').replace(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi,'[redacted-email]').slice(0,240);
const safeCode=data=>String(data?.code||data?.detail?.code||data?.type||'').slice(0,80);
const stageLog=(reasonSource,stage,status,data={},extra={})=>console.info(JSON.stringify({event:'reason_stage',reason_source:reasonSource,stage,upstream_http_status:Number(status)||null,error_code:safeCode(data)||null,error_message:status>=400?safeMessage(errorDetail(data)||data?.message||'Upstream request failed'):null,timestamp:new Date().toISOString(),...extra}));

async function loadMemory(req,res,reasonSource){
  ...
  for(let page=0;page<10;page++){
    ...
    stageLog(reasonSource,'memory_load',r.status,d,{page:page+1,item_count:Array.isArray(d.items)?d.items.length:0,profile_revision:Number(d.profile_revision||0)||null,index_status:String(d.index_status||'unknown')});
...
async function resolvePersistentChat(req,res,reasonSource){
  const r=await ixoFetch(req,res,'/chats?limit=200&offset=0',{method:'GET'}),d=await r.json().catch(()=>({}));
  stageLog(reasonSource,'persistent_chat_list',r.status,d,{chat_count:Array.isArray(d.items)?d.items.length:0});
  ...
  for(const chat of chats){
    ...
    stageLog(reasonSource,'persistent_chat_run_history',rr.status,rd,{persistent_chat_id:String(chat.id),run_count:Array.isArray(rd.items)?rd.items.length:0});
...
async function startRun(req,res,chatId,prompt,reasonSource){
  const rr=await ixoFetch(req,res,'/chats/'+encodeURIComponent(chatId)+'/runs',{method:'POST',headers:{'content-type':'application/json','X-UI-Locale':'en','Idempotency-Key':String(req.headers['x-idempotency-key']||randomUUID())},body:JSON.stringify({prompt,model_config_id:null,atomus_model_id:null,max_steps:20,adaptive_step_limit:false,attachment_ids:[],locale:'en'})}),run=await rr.json().catch(()=>({}));
  stageLog(reasonSource,'run_create',rr.status,run,{persistent_chat_id:String(chatId),run_id:run?.id?String(run.id):null,run_status:run?.status?String(run.status):null});
  if(rr.ok)stageLog(reasonSource,'run_created',rr.status,{}, {persistent_chat_id:String(chatId),run_id:run?.id?String(run.id):null,run_status:run?.status?String(run.status):null});
...
function instructionFor(mode){if(mode==='today')return'CURRENT TASK = TODAY. Answer: What deserves my attention? Return 1-5 meaningful items only when supported. An empty items array is a legitimate zero only after evaluating the supplied relevant context. Do not create urgency merely because a memory exists.';if(mode==='mirror')return'CURRENT TASK = MIRROR. Look only for observable contradiction, missing information, neglected priority (only with activity evidence), dependency, stale assumption, loose end, or goal conflict. No diagnosis or mind-reading. An empty findings array is valid.';if(mode==='decision')return'CURRENT TASK = DECIDE. Help the user see this supplied decision clearly. Do not choose for the user. The decision text is sufficient input; desired outcome, options, and assumptions are optional enrichment. If optional fields are absent, do not invent them as user-entered facts. You may identify candidate options or assumptions only as derived reasoning. Distinguish WHAT iXo KNOWS from WHAT INZO INFERRED and IMPORTANT UNKNOWNS. A nonempty summary is required. Also include at least one meaningful structured analysis section among known context, goals, constraints, options, assumptions, unknowns, tradeoffs, risks/dependencies, what would change the analysis, or clarifying questions. Other enrichment arrays may be omitted when unsupported or not useful. Any listed enrichment field you do return must use the requested value type. No prose or Markdown outside the JSON object.';return'CURRENT TASK = ASK. Answer the user actual question below. Use supplied personal context only when it materially improves the answer; otherwise say it did not materially change the answer.'}
...
export default async function handler(req,res){
 ...
 try{
  const loaded=await loadMemory(req,res,reasonSource),desired=String(req.body?.desired_outcome||'').trim(),options=Array.isArray(req.body?.options)?req.body.options.map(String).filter(Boolean).slice(0,12):[],assumptions=Array.isArray(req.body?.assumptions)?req.body.assumptions.map(String).filter(Boolean).slice(0,20):[];
  ...
  const resolved=await resolvePersistentChat(req,res,reasonSource),chatId=String(resolved.chat.id);
  const ar=await ixoFetch(req,res,'/chats/active',{method:'GET'}),ad=await ar.json().catch(()=>({}));stageLog(reasonSource,'active_chat_preflight',ar.status,ad,{persistent_chat_id:chatId,active_chat_count:Array.isArray(ad.chat_ids)?ad.chat_ids.length:0});if(!ar.ok)throw new Error(errorDetail(ad)||'Could not inspect active iXo chats');
  ...
  const attempt=await startRun(req,res,chatId,prompt,reasonSource),rr=attempt.response,run=attempt.run;
